...
ATwinLinkScaleGizmo::ATwinLinkScaleGizmo() {
...
void ATwinLinkScaleGizmo::UpdateGizmoSpace(ETwinLinkSpaceType SpaceType) {
    //Force to always be Local
    SetActorRelativeRotation(FQuat(EForceInit::ForceInit));
}

FTransform ATwinLinkScaleGizmo::GetDeltaTransform(const FVector& LookingVector
...
FTransform ATwinLinkScaleGizmo::GetSnappedTransform(FTransform& outCurrentAccumulatedTransform
...
FTransform ATwinLinkScaleGizmo::GetSnappedTransformPerComponent(const FTransform& OldComponentTransform
    , const FTransform& NewComponentTransform, ETwinLinkTransformationDomain Domain
    , float SnappingValue) const {
    ...
    if (!newScale.Equals(OldComponentTransform.GetScale3D(), 0.0001f)) {
        FVector domainScale;
        switch (Domain) {
        case ETwinLinkTransformationDomain::TD_X_Axis: domainScale = FVector(1.f, 0.f, 0.f); break;
        case ETwinLinkTransformationDomain::TD_Y_Axis: domainScale = FVector(0.f, 1.f, 0.f); break;
        case ETwinLinkTransformationDomain::TD_Z_Axis: domainScale = FVector(0.f, 0.f, 1.f); break;

        case ETwinLinkTransformationDomain::TD_XY_Plane: domainScale = FVector(1.f, 1.f, 0.f); break;
        case ETwinLinkTransformationDomain::TD_YZ_Plane: domainScale = FVector(0.f, 1.f, 1.f); break;
        case ETwinLinkTransformationDomain::TD_XZ_Plane: domainScale = FVector(1.f, 0.f, 1.f); break;

        case ETwinLinkTransformationDomain::TD_XYZ: domainScale = FVector(1.f, 1.f, 1.f); break;
        }
